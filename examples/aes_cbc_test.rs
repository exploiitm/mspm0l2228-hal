...
fn main() -> ! {
    ...
    let aesadv = mspm0l2228_hal::aes::AesAdv::new(p.aesadv, key).to_cbc();
    ...
    for _ in 0..5 {
        ...
        for i in 0..4 {
            ...
            iv[i] = trng.trng_gen_u32();
        }

        let mut res = [0x00; 4];
        aesadv.encrypt(&arr, &mut res, &iv);
        send_blocks(&uart, &iv);
    ...
    for _ in 0..5 {
        ...
        for i in 0..16 {
            arr[i] = trng.trng_gen_u32();
            iv[i % 4] ^= trng.trng_gen_u32();
        }
        let mut res = [0; 16];
        aesadv.encrypt(&dma, &arr, &mut res, &iv).unwrap();
        send_blocks(&uart, &iv);
        send_blocks(&uart, &arr);
        send_blocks(&uart, &res);
    }
    ...
    for _ in 0..5 {
        ...
        let mut res = [0; 16];
        aesadv.decrypt(&dma, &arr, &mut res, &iv).unwrap();
        send_blocks(&uart, &iv);
        send_blocks(&uart, &arr);
        send_blocks(&uart, &res);
    }
