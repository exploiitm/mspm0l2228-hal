...
impl Uart1 {
    pub fn new(uart: pac::Uart1, iomux: &pac::Iomux) -> Self {
        iomux.iomux_pincm(18).write(|w| {
            unsafe { w.pf().bits(0x2) }; // UART peripheral selection
            w.pc().connected()
        });

        iomux.iomux_pincm(19).write(|w| {
    ...
    fn init(uart: &pac::Uart1) {
        ...
        uart.uart1_lcrh().write(|w| w.brk().disable());

        uart.uart1_ctl0().write(|w| {
            match config.mode {
    ...
    fn enable(uart: &pac::Uart1) {
        uart.uart1_ctl0().modify(|_, w| w.enable().enable());
    }

    fn disable(uart: &pac::Uart1) {
        uart.uart1_ctl0().write(|w| w.enable().disable());
    }

    fn reset(uart: &pac::Uart1) {
        uart.uart1_gprcm(0).uart1_rstctl().write(|w| {
    ...
    fn pwren(uart: &pac::Uart1) {
        uart.uart1_gprcm(0).uart1_pwren().write(|w| {
            w.enable().enable();
            w.key_unlock().unlock()
        });
    }
